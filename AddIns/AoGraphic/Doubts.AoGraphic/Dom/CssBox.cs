...
using Doubts.AoGraphic.Entities;
using Doubts.AoGraphic.Handlers;
using Doubts.AoGraphic.Parse;
using Doubts.AoGraphic.Utils;

namespace Doubts.AoGraphic.Dom
{
    /// <summary>
    /// CSS盒子类
    /// </summary>
    ...
    internal class CssBox : CssBoxProperties, IDisposable
    {
        ...
        private SubString _text;//当前对应的文本
        ...
        public CssBox ContainingBlock
        {
            get
            {
                ...
                while (!box.IsBlock && box.Display != CssConstants.ListItem && box.Display != CssConstants.Table && box.Display != CssConstants.TableCell && box.ParentBox != null)
                {
                    box = box.ParentBox;
                }
        ...
        protected virtual void PerformLayoutImp(IGraphics g)
        {
            ...
            if (IsBlock || Display == CssConstants.ListItem || Display == CssConstants.Table || Display == CssConstants.InlineTable || Display == CssConstants.TableCell)
            {
                ...
                {
                    float width = ContainingBlock.Size.Width - ContainingBlock.ActualPaddingLeft - ContainingBlock.ActualPaddingRight - ContainingBlock.ActualBorderLeftWidth - ContainingBlock.ActualBorderRightWidth;
